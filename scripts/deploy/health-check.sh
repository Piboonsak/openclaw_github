...
# Usage: bash scripts/deploy/health-check.sh [URL] [MAX_RETRIES] [RETRY_INTERVAL]
set -euo pipefail
...
for i in $(seq 1 "$MAX_RETRIES"); do
  HTTP_CODE=$(curl -sf -k -o /dev/null -w "%{http_code}" "$URL" 2>/dev/null || echo "000")
  if [[ "$HTTP_CODE" == "200" ]]; then
    BODY=$(curl -sf -k "$URL" 2>/dev/null || echo "{}")
