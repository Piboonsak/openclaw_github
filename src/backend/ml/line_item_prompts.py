"""Prompt helpers for invoice line-item extraction.

Originally authored for the TASK-906 line-item PoC (`scripts/line_item_poc.py`);
relocated here as the canonical production home when the line-item stage was
wired into `run_pipeline` (W5-EXPORT-LINEITEM-REALDATA-04). `scripts/line_item_prompts.py`
now re-exports from this module for backward compatibility with the PoC harness.

Pure module — only depends on `json`/`re`, safe to import from the pipeline.
"""
